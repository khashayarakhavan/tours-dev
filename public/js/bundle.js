...
var showAlert = function showAlert(type, msg) {
  ...
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</div>"); // set an alert div using 'type' to add different classes to the html and then have different CSS styles based on this type to differentiate between success and error alerts.
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 11:
            ...
            (0, _alerts.showAlert)('error', _context.t0.response.data.message); // alert error if sth went wrong
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 8:
            ...
            (0, _alerts.showAlert)('error', _context.t0); // alert error if sth went wrong
...
var stripe = Stripe("pk_test_51GvgM7Bg5OGP7RJDsP8v6xgwZJJ7Secx9DMgo2AtlsCEZfJjBfndkAe9MrJfGTlrSYBHqTsTOeibT0scv0s8kdzy00IFAFY5aB");
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            console.log('dalam :D'); // 1) Get checkout session from API

            _context.next = 4;
            return (0, _axios.default)("/api/v1/bookings/checkout-session/".concat(tourId));

          case 4:
            clientSession = _context.sent;
            // console.log(session);
            // 2) Send the Id created at `serverSession` to stripe checkout page and charge the bill.
...
if (mapBox) {
  var locations = JSON.parse(mapBox.dataset.locations); // import locations data embeded inside dataset attribute of 'map' element.
...
if (loginForm) {
  loginForm.addEventListener('submit', function (e) {
    ...
    (0, _login.login)(email, password); // use Login function from login.js
  });
}
